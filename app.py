...
# 1. 系統初始化與 UI 設定
# ==========================================
st.set_page_config(page_title="HIOS Wave Radar V18.1", layout="wide")
st.title("🌊 HIOS Wave Radar V18.1 - 機構級量化雷達")
...
def v18_funnel_filter_and_score(df, intraday_mode):
    """V18.1 機構級漏斗篩選與評分 (支援盤中模式)"""
    filtered_df = df.copy()
    
    # --- 第一階段：絕對鐵門 (Hard Filters) ---
    # 1. 籌碼與趨勢鐵門 (無論盤中盤後都嚴格執行)
    ...
    # 2. 動能與周轉率鐵門 (若為盤中模式，則跳過這兩道鐵門避免誤殺)
