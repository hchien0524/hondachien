...
import yfinance as yf  # 📈 新增：用來自動抓取大盤指數
...
# ==========================================
# 🚨 V31 全自動大盤風控系統 (Market Risk Control)
# ==========================================
st.markdown("---")
st.subheader("🚨 戰情室：大盤風控與資金水位燈號")

@st.cache_data(ttl=300) # 快取 5 分鐘，避免頻繁向 Yahoo 請求被鎖
...
if current_close:
    ...
    if current_close > ma20:
        ...
        advice = "⚠️ 大盤跌破月線！建議降至 3 成以下資金，嚴格執行停損，多看少做，保留現金實力。"
...
with tab3:
    ...
    st.info("💡 總司令，請將您原本 V30.6 的 CSV 上傳與海選程式碼貼在此處。")

with tab4:
    st.subheader("🛡️ 持股監控中心")
    st.info("💡 總司令，請將您原本的持股監控程式碼貼在此處。")
